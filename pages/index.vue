...
<template>
  <section class="space-y-5">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-semibold text-emerald-900">Beet-Übersicht</h2>
      <NuxtLink class="rounded-full bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700" to="/beds">
        + Beete verwalten
      </NuxtLink>
    </div>
    ...
    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      <article v-for="item in summary" :key="item.bed.id" class="card-soft p-4">
        <div class="mb-3 flex items-start justify-between gap-2">
          ...
          <NuxtLink
            :to="`/beds/${item.bed.id}`"
            class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800 hover:bg-emerald-100"
          >
            Details
          </NuxtLink>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div
            ...
            class="grid grid-cols-[4.2rem_1fr] items-start gap-2 rounded-lg border border-emerald-100 bg-emerald-50/40 p-2"
          >
